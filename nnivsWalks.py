...
def randNNIwalk(daf1,daf2,size,steps,runs,seed,weighted=False):
    global __pid__
...
if __name__=='__main__':
    if len(sys.argv)<6:
        ...
        print ("Usage: [-w] <GR GL or RL> <size or size range> <no. NNI steps> <no. runs> <seed or seed range>")
        sys.exit(-1)
    ...
    for size in size_range:
        for seed in seed_range:
            randNNIwalk(dist_algo_file1,dist_algo_file2,size,steps,runs,seed,WEIGHTED)